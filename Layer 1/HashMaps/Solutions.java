public class Solution{
    ...
    public List<String> repeatedSequence(String s){
        ...
        return new ArrayList(result);       //! Make ArrayList out of a HashSet
    }
    ...
    public List<Integer> topKFrequent(int[] nums, int k){
        ...
        PriorityQueue<Map.Entry<Integer, Integer>> pq = new PriorityQueue((a, b) -> (a.getValue() - b.getValue()));     //! This gives a max heap.
        ...
        // Poll top k elements and add them to the result ArraList
        for(int i = 0; i < k; i++){
    ...
    // Count primes upto the given integer n
        ...
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < cols; j++){
                if(matrix[i][j] == 1){
                    ...
                    if(j < col - 1 && matrix[i][j + 1] == 1) neighbours++;
                    if(i < rows - 1 && matrix[i + 1][j] == 1) neighbours++;
                }
            }
        }
